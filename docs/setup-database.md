...
**Points and login count:** Add columns for referral points, admin bonus points, activity points, and login count:
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS activity_points INTEGER NOT NULL DEFAULT 0;
```

The API updates `last_seen_at` and increments `login_count` on every `/auth/sync`. Referral completion credits `referral_points` to the referrer; admins grant `bonus_points` via `POST /admin/points`. **Activity points** are earned from day streak and check-ins: when the app calls `GET /users/me/points?streak=&checkIns=`, the API updates `activity_points` (2 points per streak day, 1 per check-in) and returns the full breakdown. If your `users` table already had the points columns from an earlier setup, run only: `ALTER TABLE users ADD COLUMN IF NOT EXISTS activity_points INTEGER NOT NULL DEFAULT 0;`
