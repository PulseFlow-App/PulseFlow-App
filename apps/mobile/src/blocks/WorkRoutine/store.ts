...
export function getHistoricalCheckIns(days: number): CheckInEntry[] {
...
export function getWeeklyProgress(): { count: number; percent: number } {
  const week = getHistoricalCheckIns(7);
...
/** Reset store state. For tests only. */
export function resetWorkRoutineStore(): void {
  entries = [];
}
